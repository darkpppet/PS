...
int main(void)
{
    ...
    vector<complex<double>> seqS(1048576);
    vector<complex<double>> seqT(1048576);

    for (int i = 0; i < n; i++)
    {
        seqS[i] = complex<double>(s[i] - '0');
        seqS[i + n] = complex<double>(s[i] - '0');
        seqT[n - i - 1] = complex<double>(t[i] - '0');
    }
    ...
    for (int i = 0; i < 1048576; i++)
        resultSeq[i] = seqS[i] * seqT[i];

    ifft(resultSeq);

    int result = 0;
    for (int i = n - 1; i < 2 * n; i++)
        result = max(result, (int)round(resultSeq[i].real() / 1048576));
